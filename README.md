...
| **Live** | Enter your smart-mock `user_id` + token → fetches your real tables. |
...
Link `orders.customer_id` → `customers.id` via a cable to get relational data.

Full setup guide: [`schema/setup.md`](schema/setup.md)

### 3. Connect to live data

Paste your `user_id` and `smt_` token into the config bar at the top of the dashboard and click **↻ fetch live data**.
